...
class Env():
            ...
            for i in range(HEIGHT):
                for j in range(WIDTH):
                    if self.cur_position[0:2] == [i,j]:
                        ...
                        img = self.shapes[3]
                    elif self.map[i,j] == self.obstacle:
                        img = self.shapes[4]
    ...
    # obstacle을 만들어줌(x: 왼쪽 위 x좌표, y: 왼쪽 위 y좌표, a: x방향 길이, b: y방향 길이)
    def obstacle_maker(self, x, y, a, b):
        for i in range(a):
            for j in range(b):
                ob_x = x + i
                ob_y = y + j
                self.map[ob_x, ob_y] = self.obstacle
        self.obstacle_num += a*b


    def reset(self):
        self.map = np.zeros([HEIGHT,WIDTH])
        self.cur_position = [19,19,2]
        self.obstacle_num = 0
        self.map[self.cur_position[0],self.cur_position[1]] = self.cleaned

        # 장애물 구조
        self.obstacle_maker(6,0,1,9)
        self.obstacle_maker(0,11,9,1)               
        self.obstacle_maker(16,7,4,1)             
        self.obstacle_maker(13,0,1,8)
        self.counter = 0
        self.combo = 0
    ...
    def get_done(self):
        if self.map.sum() == self.obstacle_num*(self.obstacle-self.cleaned) + HEIGHT*WIDTH*self.cleaned:
            return True
        if self.counter >= 500:
            return True
        return False
